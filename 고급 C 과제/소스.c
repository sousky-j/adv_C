...
void sort_sub()
{
	...
	while (1)
	{
		if (feof(rfp) != 0) break;//파일 끝에 도달하면 loop 탈출
		...
		if (head == NULL)	head = new_node;//첫 노드를 head와 연결
		else
		{
			...
			while (1)
			{
				...
				if (strcmp(new_node->sub, tmp->sub) < 0)//연결할 노드가 비교 노드보다 앞에 위치할 때
				{
					...
					if (tmp == head)   head = new_node;//비교 노드가 head면 head 포인터가 연결할 노드를 가리킴
					else   pre->next = new_node;//비교 노드가 head가 아니면 비교 노드의 이전 노드가 연결할 노드를 가리킴
					break;//연결할 노드 연결 시 loop 탈출
				}
				else if (strcmp(new_node->sub, tmp->sub) == 0)//연결할 노드와 비교 노드가 같은 과목일 때
				{
					while(1)
					{
						...
						{
							if (pre != NULL) pre = pre->next;//tmp 이전 노드의 노드 이동
							else if (pre == NULL) pre = head;//pre를 tmp보다 한칸 뒤에 오게 함
...
void sort_stu()
{
	...
	while (1)//버블 정렬 밖 loop문
	{
		...
		{
			if (cur->next == NULL) break;//비교 노드가 마지막에 도달하면 loop 탈출
			if (strcmp(cur->name, cur->next->name) > 0)//비교 노드,비교 노드의 다음 노드가 바뀔 때
			{
				...
				//=========================//
				if (pre != NULL) pre->next = tmp;//이전 노드가 있으면 비교 노드의 이전 노드를 현재 비교 노드로 설정
				else head = tmp;//이전 노드가 없으면 head 포인터가 스왑 이후 앞으로 옮겨진 노드를 가리킴
		...
		if (flag==0) break;//정렬이 끝나면 loop 탈출
	}
}
...
void memory_manager()
{
	...
	while (pre != NULL)//마지막 노드를 지나면 loop를 탈출함
	{
		...
		if (tmp == NULL) break;//pre가 마지막 노드를 해제한 후, tmp가 NULL이면 loop를 탈출함
